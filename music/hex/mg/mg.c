/******************************************************************
 * Denne version af musicafspiller er inspireret af:
 * 
 * Melody Generator som kan findes på websiden:
...
/******************************************************************
 * Konverteret fra Melody Generator Include File: mg.inc
 ******************************************************************/
...
#define	EoS     255	//End of score
#define	en      128	//End of line flag
...
uint8_t NoteIdx = 1; // Note rotation index
uint8_t Notes   = (2+3+1+1+1+1)*N_NOTE
#define	ns_freq = 0	//Angular Speed
#define	ns_rptr = 2	//Wave table read pointer (16.8 fraction)
#define	ns_lvl = 5	//Level
#define	ns_wrap = 6	//Loop Flag
#define	ns_loop = 7	//Loop Count
#define	ns_lp = 8	//Level Pointer
#define	nsize = 9	//size of this structure
 ...
 * The wavetable synthesizer synthesizes a sound with pre-sampled 
 * (pre-synthesized) waveform. This is a method that have established 
 * at dawn of digital synthesizer. MOD Sound on AMIGA is the first time 
 * that wavetable synthesizer was realized on the PC if my memory serves. 
 * Ofcourse the wavetable synthesizer is embedded as a sound driver into 
 * recent PCs. The theory of wavetable synthesizer is very simple and its 
 * processor load is relatively low. However, it requires large amount of 
 * memory because there are wavetable data for source of sound. Therefore, 
 * following technics are applied to existing wavetable synthesizer.
 * 
 * Not reserve wavetables for each tone. It can also synthesize any tone 
 * with a wavetable with changing play back speed.
 * 
 * Not reserve all part of wavetable. Monotonous part can be replaced with 
 * a loop.
 * 
 * Decaying waveform can be simulated in envelope genarotor.
 ******************************************************************/
 ...
 ******************************************************************
 * Since sustain area of wave table, a cycle of fundamental frequency, 
 * is sampled in 128 points, the base frequency becomes 32000/128 = 250 Hz. 
 * The wave table lookup pointer, 16.8 fraction, is increased every sample 
 * by these 8.8 fractional angular speed values.
 ******************************************************************/
